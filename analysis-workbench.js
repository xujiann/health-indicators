(() => {
  ...
  function draw() {
    ...
    host.innerHTML = '<div class="wb-shell" role="dialog" aria-modal="true" aria-labelledby="wbTitle"><div class="wb-head"><div class="wb-title" id="wbTitle">数据分析工作台</div><button class="wb-close" type="button" aria-label="关闭分析工作台">×</button></div><div class="wb-tabs" aria-label="分析模块">' + tabs.map(tab => '<button type="button" class="wb-tab' + (tab[0] === active ? ' on' : '') + '" data-wb-tab="' + tab[0] + '" aria-pressed="' + (tab[0] === active ? "true" : "false") + '">' + tab[1] + '</button>').join("") + '</div><div class="wb-body">' + panels[active]() + '</div></div>';
  ...
  function qualityPanel() { const rows = rowsNow(), nondirect = rows.filter(row => sourceStatus(row).cls !== "link").length, review = rows.filter(row => sourceStatus(row).cls === "review").length, ids = {}; rows.forEach(row => { const id = [row.region,row.year,row.compare_key,row.nature].join("|"); ids[id]=(ids[id]||0)+1; }); const duplicate = Object.values(ids).filter(n => n>1).reduce((sum,n) => sum+n-1,0); return '<div class="wb-grid"><div class="wb-card"><div class="wb-value">' + rows.length + '</div><p>当前范围记录</p></div><div class="wb-card"><div class="wb-value">' + nondirect + '</div><p>非单条原文</p></div><div class="wb-card"><div class="wb-value">' + review + '</div><p>待复核材料记录</p></div><div class="wb-card"><div class="wb-value">' + duplicate + '</div><p>同口径同年重复</p></div></div><div class="wb-card" style="margin-top:12px"><h3>分类与可用性</h3><p>' + uniq(rows.map(row => row.category)).length + ' 个一级类别 · ' + uniq(rows.map(row => row.category + "|" + row.subcategory)).length + ' 个二级分类 · ' + uniq(rows.map(metricForm)).length + ' 种指标口径。</p><div class="wb-controls" style="margin-top:9px"><button class="wb-button light" data-wb-quality="nondirect" type="button">查看非单条原文</button><button class="wb-button light" data-wb-quality="review" type="button">查看待复核</button></div></div>'; }
  ...
  function updatePanel() { const agencies=["国家卫健委","国家医保局","国家统计局","财政部","地方部门"], indexed=DATA.filter(row=>sourceStatus(row).cls==="index").length; return '<div class="wb-grid">' + agencies.map(agency => { const rows=DATA.filter(row=>sourceAgency(row)===agency); if(!rows.length)return ""; const year=Math.max(...rows.map(row=>+row.year).filter(Boolean)), source=rows.find(row=>row.source_url); return '<div class="wb-card"><h3>' + h(agency) + '</h3><div class="wb-value">' + year + '</div><p>当前已收录最新年度</p>' + (source?'<a class="wb-link" target="_blank" rel="noopener" href="' + h(source.source_url) + '">打开最近公开源</a>':"") + '</div>'; }).join("") + '</div><div class="wb-card" style="margin-top:12px"><h3>维护闭环</h3><p>每周比较官方页面状态、标题和内容指纹；变化自动进入 GitHub 复核 Issue。当前还有 ' + indexed + ' 条记录仅关联官方来源索引。</p><p style="margin-top:8px"><a class="wb-link" target="_blank" rel="noopener" href="https://github.com/xujiann/health-indicators/actions/workflows/official-source-watch.yml">巡检记录</a> · <a class="wb-link" href="coverage.html">覆盖维护页</a> · <a class="wb-link" target="_blank" rel="noopener" href="docs/数据覆盖率报告.md">覆盖率报告</a> · <a class="wb-link" target="_blank" rel="noopener" href="docs/官方数据源巡检.md">维护说明</a></p></div>'; }
